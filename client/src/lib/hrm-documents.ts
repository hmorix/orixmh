...
function renderFooter(): string {
  return `
    <div class="footer">
      This is an official document issued by ${COMPANY.name} &bull; CIN: ${COMPANY.cin} &bull; Verification: hr@hmorix.com &bull; ${COMPANY.website}
    </div>
  `;
}
...
export function downloadTextDoc(content: string, filename: string) {
...
export function copyTextDoc(content: string): Promise<boolean> {
...
export function printOfferLetter(data: OfferLetterData) {
  ...
  const html = `
    ...
    <p>We are pleased to extend this formal offer of employment for the position of <strong>${data.role}</strong> with <strong>${COMPANY.name}</strong>. Following our evaluation and interviews, we were thoroughly impressed with your technical capabilities and believe you will make substantial contributions to our team.</p>
    ...
    <div class="terms-box">
      ...
      <ul>
        <li><strong>Probationary Period:</strong> 90 calendar days from your date of joining. Confirmation is subject to performance review.</li>
        <li><strong>Notice Period:</strong> 30 days during probation; 60 days post-confirmation of employment.</li>
        <li><strong>Validity:</strong> This offer remains valid for 7 calendar days from the date of issue.</li>
        <li><strong>Required Documents:</strong> Educational credentials, previous employment relieving letters, PAN, and Aadhaar card copies.</li>
        <li><strong>Confidentiality:</strong> The terms and compensation details of this offer are strictly confidential.</li>
      </ul>
    </div>

    <p>Please sign the duplicate copy of this letter as acceptance of this offer and return it to Human Resources within the validity period.</p>

    ${renderSignatures()}

    <div class="ack-box">
      <div class="ack-header">Candidate Acceptance & Confirmation</div>
      <p>I, <strong>${data.name}</strong>, hereby accept the terms and conditions outlined in this offer letter and confirm my date of joining as <strong>${data.joiningDate}</strong>.</p>
...
export function printJoiningLetter(data: JoiningLetterData) {
  ...
  const html = `
    ...
    <div class="meta-grid">
      ...
      <div class="meta-item"><strong>Work Location:</strong> ${data.location}</div>
      <div class="meta-item"><strong>Joining Date:</strong> ${data.joiningDate}</div>
      ${data.reportingTo ? `<div class="meta-item"><strong>Reporting Manager:</strong> ${data.reportingTo}</div>` : ""}
    </div>

    <p>Dear <strong>${data.name}</strong>,</p>
    <p>We are delighted to officially welcome you to <strong>${COMPANY.name}</strong>. We confirm that your onboarding is complete and your employment with us commences on <strong>${data.joiningDate}</strong>.</p>
    <p>You have been assigned the permanent Employee Identification number <strong>${data.employeeId}</strong>. Please use this ID for all official communications, payroll disbursements, and portal access.</p>

    <div class="terms-box">
      <h3>Workplace Standards & Operating Policies</h3>
      <ul>
        <li><strong>Official Hours:</strong> Monday through Friday, 9:30 AM to 6:30 PM (flexible subject to project requirements).</li>
        <li><strong>Self-Service Portal:</strong> Access attendance, leave applications, payslips, and OKRs at <a href="${COMPANY.website}/employee/login">${COMPANY.website}/employee/login</a>.</li>
        <li><strong>Annual Leave Entitlement:</strong> 12 Casual Leaves, 10 Sick Leaves, and 18 Earned Leaves per calendar year.</li>
        <li><strong>Asset Responsibility:</strong> Any company laptops, software licenses, or credentials allocated are strictly for official duties.</li>
        <li><strong>Information Security:</strong> Complete compliance with client non-disclosure agreements (NDAs) and company cybersecurity protocols.</li>
      </ul>
    </div>

    <p>We look forward to an enriching, high-impact career journey with us. Welcome aboard!</p>
...
export function printAppointmentLetter(data: AppointmentLetterData) {
  ...
  const html = `
    ...
    <p>Dear <strong>${data.name}</strong>,</p>
    <p>Consequent to your acceptance of our offer, the Management of <strong>${COMPANY.name}</strong> is pleased to appoint you as <strong>${data.role}</strong> in the <strong>${data.department}</strong> department, effective from <strong>${data.joiningDate}</strong>, under the following governing terms:</p>

    <div class="terms-box">
      <h3>Terms & Conditions of Employment</h3>
      <ul>
        <li><strong>Remuneration:</strong> Your agreed Annual CTC is ₹${data.ctc.toLocaleString("en-IN")} (₹${monthlyCtc.toLocaleString("en-IN")}/- monthly gross), subject to statutory tax and PF withholdings.</li>
        <li><strong>Probation & Confirmation:</strong> You shall be on probation for a period of 3 months. Confirmation will be communicated in writing upon satisfactory performance.</li>
        <li><strong>Duties & Responsibilities:</strong> You shall perform all tasks assigned with due diligence, integrity, and adherence to company quality guidelines.</li>
        <li><strong>Intellectual Property:</strong> All intellectual property, code, designs, and innovations developed during your tenure belong solely to ${COMPANY.name}.</li>
        <li><strong>Separation & Notice:</strong> Post-confirmation, either party may terminate employment by giving sixty (60) days written notice or salary in lieu thereof.</li>
      </ul>
    </div>

    <p>We trust that you will play a pivotal role in driving our company objectives to new heights.</p>

    ${renderSignatures()}

    <div class="ack-box">
      <div class="ack-header">Acknowledgement & Acceptance</div>
      <p>I have read, understood, and unconditionally agree to abide by all the terms and conditions outlined in this Appointment Letter.</p>
      <div class="ack-sig-row">
        <div>
          <div class="sig-line"></div>
...
export function printExperienceLetter(data: ExperienceLetterData) {
  ...
  const html = `
    ...
    <p style="text-align: center; font-size: 13px; font-weight: 700; margin: 20px 0; text-transform: uppercase; letter-spacing: 0.05em;">
    ...
    <p>During their tenure, they held the position of <strong>${data.role}</strong> in the <strong>${data.department}</strong> department.</p>
    ...
    <p>${data.performance || "During their employment, " + data.name + " demonstrated exemplary professional conduct, strong technical skills, and commitment to excellence. Their conduct and character were found to be very good during this period."}</p>
    
    <p>We take this opportunity to thank them for their valuable service and wish them the very best in all their future professional endeavors.</p>
...
export function printRelievingLetter(data: RelievingLetterData) {
  ...
  const html = `
    ...
    <p>Dear <strong>${data.name}</strong>,</p>
    <p>With reference to your formal resignation, we hereby accept the same and confirm that you are formally relieved from all duties and employment with <strong>${COMPANY.name}</strong> with effect from the close of business hours on <strong>${data.relievingDate}</strong>.</p>
    
    <p>We confirm that you have successfully completed all exit handovers, returned all company-issued assets, hardware, access cards, and credentials, and cleared all departmental dues.</p>
    <p>Your full and final settlement (FnF) statement and dues have been processed as per company policy.</p>

    <p>We appreciate your contributions during your tenure and wish you great success in your future career.</p>
...
export function printPayslip(data: PayslipData) {
  ...
  const html = `
    ...
    <p style="font-size: 9.5px; color: #888888; text-align: center; margin-top: 14px;">
      This is a digitally generated document created by the HMorix HR Management System. No physical signature required. For discrepancies, contact hr@hmorix.com within 5 business days.
    </p>
  `;
